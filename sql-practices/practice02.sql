...
-- 		예) 2014년 07월 10일
-- 2000년 01월 28일
select min(hire_date),date_format(max(hire_date),'%Y년 %m월 %d일')
	from employees;

-- 문제3 : 가장 오래 근속한 직원의 입사일은 언제인가요? 다음 형식으로 출력해주세요.
-- 		예) 2014년 07월 10일
-- 		2001년 08월 02일
select date_format(min(from_date),'%Y년 %m월 %d일') as '가장 오래 근속한 직원'
	from salaries
...
-- 최고 어린 사원 : 1965-02-01 / 최 연장자 : 1952-02-01
select  2023 - YEAR(min(birth_date))+1, 2023 - YEAR(max(birth_date))+1 
		from employees;
