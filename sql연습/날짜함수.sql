...
-- 날짜를 date 타입의 컬럼이나 값에 type(year, monthm day)의 표현식으로 더하거나 뺄 수 있다.
-- 예) 각 사원등의 근속년 수가 5년이 되는 날에 휴가를 보내준다면 각 사원의 5년 근속 휴가 날짜는?
...
select '12345' +10,
 ...
 from dual;
select date_format(cast('2023-01-09' as date), '%Y년 %m월 %d일'); 
...
-- type
-- 문자 : varchar, char, text, CLOB(Character Large Obejct)
-- 정수 : medium int, int(signed, integer), unsigned, big int
-- 실수 : float, double
-- 시간 : date, datetime
-- LOB : CLOB, BLOG(Binary Large Object)